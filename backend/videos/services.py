...
class VideoProcessingService:
    ...
    def _store_in_weaviate(self, video, keyframes, keyframes_dir):
        ...
        try:
            ...
            # Add color references for dominant colors
            for color in colors:
                rgb = color['rgb']
                rgb_vector = [rgb[0] / 255, rgb[1] / 255, rgb[2] / 255]  # Normalize to [0,1]

                color_uuid = weaviate_client.add_color_reference(
                    mongodb_id=str(video.id),
                    video_id=video_uuid,
                ...
                if color_uuid:
                    # Create VectorReference for color
                    VectorReference(
                        content_type='color',
                        object_id=str(video.id),
                        weaviate_id=color_uuid,
                        vector_type='color',
                        parent_video_id=str(video.id)
                    ).save()
